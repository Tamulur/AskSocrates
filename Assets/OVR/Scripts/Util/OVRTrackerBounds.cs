...
public class OVRTrackerBounds : MonoBehaviour
{
    ...
    void Start()
    {
		...
		OVRManager.display.UpdatedTracking += OnTrackingUpdated;
    }
	...
	/// <summary>
	/// Computes frustum planes from sensor frustum parameters.
	/// </summary>
	void ComputePlanes()
    {
	...
	void OnTrackingUpdated ()
	{
		...
		var q = trackerPose.orientation;

		//HACK: Persistent race condition.
		if (Mathf.Approximately(q.x, 0f) &&
		    Mathf.Approximately(q.y, 0f) &&
		    Mathf.Approximately(q.z, 0f) &&
		    Mathf.Approximately(q.w, 0f))
			trackerPose.orientation = Quaternion.identity;
